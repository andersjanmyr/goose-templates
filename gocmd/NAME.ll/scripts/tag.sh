...
sed -i.bak -E "s/v[0-9]\.[0-9]\.[0-9]/$new_version/g" pkg/{{lowercaseletters .NAME}}/version.go
git add pkg/{{lowercaseletters .NAME}}/version.go
git commit -am "Changed version to $new_version"
git tag v${new_version} -am "Release $new_version"

rm pkg/{{lowercaseletters .NAME}}/version.go.bak
